...
function objToHTML(obj) {
    if (!obj || !obj.socket) return;
    var keys = obj.socket.server.stack;
    console.log('keys');
    console.log(keys);
    keys.forEach(function(ob, index) {
        var json = sys.inspect(ob, true, 100, true);
        console.log(index + ' ***** ' + json);
        //var cleanObj = JSON.parse(json);
    });
    //var json = inspect(obj, false, false);
    //var cleanObj = JSON.parse(json);
    //return OLD_jsonToHTML(cleanObj);
}

function inspectt(obj, showHidden, depth) {
    var seen = [];
