...
function compare(options) {
    options.before.each(function(value) {
        if (!options.after.indexOf(value)){
            log(options.removed + ':', value)
        }
    });


    options.after.each(function(value) {
        if (!options.before.indexOf(value)){
            log(options.added + ':', value)
        }
    });
...
function changelog(new_data) {
    ...
    compare({
        before: jobData.critical,
        after: new_data.critical,
        added: 'new critical',
        removed: 'no longer critical'});
}
