...
function format_results(data) {
    ...
    data.forEach(function(job) {
        ...
        if (all_urls[urls.long_url]) {
            log('Duplicate:', job.id, urls.long_url, 'of', all_urls[urls.long_url]);
            all_urls['/' + job.id] = all_urls[urls.long_url];
        }
        else {
            ...
            jobs[location][team].push(job.id);
        }
    });
    ...
    jobs.forEach(function(location, location_key){
        jobs[location_key].forEach(function(team, team_key) {
            ...
            jobs[location_key][team_key].forEach(function(job_id) {

                all_ids[job_id].similar = similar(all_ids[job_id]);
            });
        });
    });
...
function init(new_data) {
    ...
    log('init complete', 'total jobs:', Object.keys(all_ids).length, 'critical:', all_critical.length);
}
