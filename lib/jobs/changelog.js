...
function changelog(old_data, new_data) {
    ...
    // other job changes
    var title = function(job) { return job.title + ' (' + Constants.short(job.location) + ', ' + (job.critical ? 'CRITICAL, ' : '') + 'http://opower.jobs/' +  job.id + ')';};
    diff = compare({
    ...
    if (results.length) {
        ...
        results.forEach(function(item) { log(item); });
    }
