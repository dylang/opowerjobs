...
function create_job_url(job) {
    ...
    var long_url = [ '/jobs', location_url, department_url, title_url ].join('/');
    var apply_url = [ '/jobs/apply', location_url, department_url, title_url ].join('/');
...
function format_results(data) {
    ...
    data.forEach(function(job) {
        ...
        if (!all_urls[urls.long_url]) {
            ...
            if (!all_urls['/jobs/' + urls.title]) {
                all_urls['/jobs/' + urls.title] = job.id;
            }
...
function similar(job) {
    ...
    var title = job.title;
    var search_for = (title.search(/professional services/i) !== -1) ? 'professional services'
            : (title.search(/engineer/i) !== -1) ? 'engineer'
            : (title.search(/director/i) !== -1) ? 'director'
            : 'no matches';

    var matches = [];
    for (job_id in all_ids) {
        var j = all_ids[job_id];
        if (j.id != job.id && j.title.toLowerCase().search(search_for) !== -1) {
            matches.push(j);
        }
    }
