...
function format_description(desc) {
    var j1 = desc.split(/(Job Description)|(About the Role)|(ABOUT THE JOB)|(Responsibilities)|(REQUIREMENTS)|(About You)/i);
    var j2 = [];
    var d = [];
    ...
    for (i=0, l=j2.length; i<l; i++) {
        if (j2[i].length) {
            // wrap P with <p> only if it doesn't start with a <ul>
