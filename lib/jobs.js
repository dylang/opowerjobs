...
var connect = require('connect'),
    ...
    function search(req, res) {
        ...
        jobStore.all(function (doc) {
            ...
            res.render(view_dir + '/index.ejs', { locals: { jobs: search_results, query: query } });
    ...
    function job_location(req, res, next) {
        ...
        if (jobs.locations[location]) {
            ...
            res.render(view_dir + '/index.ejs', { locals: { jobs: local_jobs } });
        }
        else {
            next();
        }
    ...
    function render_list(req, res, next) {
        ...
        if (location && jobs.locations[location]) {

            dept_jobs.locations[location] = {};
            dept_jobs.locations[location][department] = jobs.locations[location][department];
        } else {
            jobs.locations.forEach(function(data, location) {
                log(location);
                if (jobs.locations[location][department]) {
                    dept_jobs.locations[location] = {};
                    dept_jobs.locations[location][department] = jobs.locations[location][department];
                }
            });
        ...
        res.render(view_dir + '/index.ejs', { locals: { jobs: dept_jobs, location: location, department: department } });
    }
