...
function render_search(req, res, next) {
    ...
    res.render(view_dir + '/search.ejs', { locals: { jobs: jobs, query: query, search_log: jobData.search_log, currengeID: 'jobs' } });
}
...
function render_new(req, res, next) {
    res.render(view_dir + '/new.ejs', { locals: { currentPageID: 'jobs' } });
}

//app.get('/hot'
function render_hot(req, res, next) {
    res.render(view_dir + '/hot.ejs', { locals: { currentPageID: 'jobs' } });
}
...
function render_department(req, res, next) {
    ...
    if (location && !jobData.all_locations[location] && jobData.all_teams[department]) {
        res.redirect('/' + department);
    }

    if (!jobData.all_teams[department]) {
        next();
    }
    else {
        ...
        res.render(view_dir + '/team.ejs', { locals: { header: dept_file, jobs_sf: jobs_sf, jobs_dc: jobs_dc, department: department, currentPageID: 'jobs' } });
    }

}

function lookup_longurl(req) {
    log('lookjup long url');
    var job,
...
function render_teams(req, res) {
    res.render(view_dir + '/teams.ejs', { locals: { currentPageID: 'jobs' } } );
}
