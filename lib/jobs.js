...
function render_department(req, res, next) {
    ...
    if (!jobData.all_departments[department]) {
        ...
        var jobs_sf = jobData.all_jobs.locations['san-francisco'][department],
            jobs_dc = jobData.all_jobs.locations['dc-northern-virginia'][department];

        var dept_files = { engineering: 'engineering' };
        var dept_file = dept_files[department] || 'generic-department';
        log (dept_file);
        res.render(view_dir + '/departments/' + dept_file + '.ejs', { locals: { jobs_sf: jobs_sf, jobs_dc: jobs_dc, department: department, currentPageID: 'jobs' } });
    }
