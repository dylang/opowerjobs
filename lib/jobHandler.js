...
function jobvite_redirect(req, res, next) {
    if (req.query && req.query.jvi) {
        log('Jobvite', req.query.jvs, req.headers.referrer || req.headers.referer || '');
        var request = req.query.jvi.split(','),
            job_id = request[0] ? jobData.all_urls['/' + request[0]] : false,
            job = job_id ? jobData.all_ids[job_id] : false,
            action = request[1] ? request[1] : false;

        if (job) {
            if (req.session) {
                req.session.title = job.title;
                req.session.location = job.location;
            }
            res.redirect( (action == 'Apply' ? '/apply' : '') + job.url.long_url);
        } else {
            log('jobvite 404:', request[0]);
            next();
        }
    }
    else {
