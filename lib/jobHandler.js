...
function render_apply(req, res, next) {
    if (req.session.jvs || req.session.insider) { log('Apply', 'Generic', req.session.jvs || '', req.session.insider || ''); }
...
function render_apply_job(req, res, next) {
    ...
    else
    {
        job = lookup_longurl(req);
        if (job) {
            if (req.session.jvs || req.session.insider) { log('Apply', job.url.long_url, req.session.jvs || '', req.session.insider || ''); }

            res.render(view_dir + '/jobvite-apply.ejs', { locals: { job: job, title: 'Apply', referral: ReferralHandler.generateString(req.session), currentPageID: PAGE_ID } });
        } else {
            next();
        }
    }
...
function jobvite_redirect(req, res, next) {
    if (req.query && req.query.jvi) {
        log('Redirect from Jobvite', req.query.jvs, req.headers.referrer || req.headers.referer || 'No referral');
        var request = req.query.jvi.split(','),
            job = jobData.ids[request[0]],
            action = request[1];
        if (job) {
            res.redirect('/' + (action == 'Apply' ? 'apply/' : '') + job.url.long);
        } else {
            log('jobvite query not found:', req.query);
            next();
        }
    }
    next();
