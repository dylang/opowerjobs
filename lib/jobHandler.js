...
function render_all(req, res, next) {
    res.render('job-pages/all-jobs.ejs', {
        jobs: jobData.all_jobs,
        title: 'All Openings',
        currentPageID: 'all'
    });
...
function render_apply_job(req, res, next) {
    ...
    else
    {
        job = lookup_longurl(req);
        if (job) {
            res.render(view_dir + '/jobvite-apply.ejs', {
                job: job,
                title: 'Apply',
...
function render_feedburner(req, res, next) {
    ...
    if ((req.header('user-agent','').match(/FeedBurner|FeedValidator/i) === null) && req.headers.host.split(':')[0] != 'localhost') {
