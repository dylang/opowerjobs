...
function reload_from_jobvite(req, res, next) {
    reload(function(changes) {
        res.render('reload.ejs', { title: 'Complete', message: 'Done loading jobs.', changes: changes, currentPageID: 'jobs' } );
    })
}
...
function render_search(req, res, next) {
    ...
    res.render('search/search.ejs', { jobs: jobs, query: query, title: 'Search for ' + query, currentPageID: PAGE_ID } );
}
...
function render_sf(req, res, next) {
    ...
    res.render('locations/sf.ejs', { jobs: jobs, title: Constants.long('sf'), currentPageID: PAGE_ID } );
}

function render_dc(req, res, next) {
    var jobs = jobData.all_jobs.va;
    res.render('locations/dc.ejs',{ jobs: jobs, title: Constants.long('va'), currentPageID: PAGE_ID } );
}

//app.get('/all'
function render_all(req, res, next) {
    res.render('job-pages/all-jobs.ejs', { jobs: jobData.all_jobs, title: 'All Openings', currentPageID: 'all' });
}
//app.get('/new'
function render_new(req, res, next) {
    res.render('job-pages/new.ejs', { currentPageID: PAGE_ID, title: 'New Positions' } );
}
...
function render_team(req, res, next) {
    ...
    if (!Constants.teams[team]) {
        ...
        res.render('teams/team.ejs', { team_page: team_page, jobs_sf: jobs_sf, jobs_dc: jobs_dc, team: team, currentPageID: PAGE_ID });
    }
...
function render_job(req, res, next) {
    ...
    if (job) {
        ...
        if (job) {
            res.render(view_dir + '/job.ejs', {  job: job, currentPageID: PAGE_ID } );
        } else {
            next();
        }
    }
...
function render_apply(req, res, next) {
    res.render('job-pages/jobvite-apply.ejs', { title: 'Apply', referral: ReferralHandler.generateString(req.session), currentPageID: PAGE_ID } );
}
...
function render_apply_job(req, res, next) {
    ...
    else
    {
        job = lookup_longurl(req);
        if (job) {
            res.render(view_dir + '/jobvite-apply.ejs', { job: job, title: 'Apply', referral: ReferralHandler.generateString(req.session, job), currentPageID: PAGE_ID } );
        } else {
            next();
        }
    }
...
function render_teams(req, res) {
    res.render('teams/teams.ejs', {  title: 'Teams', currentPageID: 'teams' } );
}
