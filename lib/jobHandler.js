...
function render_search(req, res, next) {
    ...
    res.render(view_dir + '/search.ejs', { locals: { jobs: jobs, query: query, currentPageID: PAGE_ID } });
}
...
function render_feed(req, res, next) {
    ...
    if (req.headers['user-agent'].match(/FeedBurner|FeedValidator/i) === null && req.headers.host.split(':')[0] != 'localhost') {
