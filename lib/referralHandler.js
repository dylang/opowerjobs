...
function insider(req, res, next) {
    ...
    if (req.session && matches && matches.length > 2) {
        ...
        log('opowerjobs ad', req.session.insider, req.session.referral, url, req.headers('user-agent'));
        res.redirect(url);
    }
    else {
        next();
...
function metro_ad(req, res, next) {
    ...
    log('DC Metro Ad', req.session.referral || '', req.header('user-agent'));
    next();
}

function sweetlife_festival(req, res, next) {
    track_insider(req, 'Sweetlife.Festival');
    log('Sweetlife Festival', req.session.referral || '', req.header('user-agent'));
    next();
}
...
function moustache_tracking(req, res, next) {
    log('Moustache', req.session.referral || '', req.header('user-agent'));
    next();
}

function mustache_tracking(req, res, next) {
    log('Mustache', req.session.referral || '', req.header('user-agent'));
    next();
}
...
function addHandlers(options) {
    ...
    Server.get('/sweetgreen', sweetlife_festival);
    Server.get('/sweetlife', sweetlife_festival);
    Server.get(/\/insider\//, insider);
    Server.get(/^/, jobviteQueryString);

    Server.get('/moustache', moustache_tracking);
    Server.get('/mustache', mustache_tracking);
