...
function insider(req, res, next) {
    ...
    if (req.session && matches && matches.length > 2) {
        ...
        log('INSIDER', req.session.insider, req.session.referral, url);
        res.redirect(url);
    }
    else {
        next();
...
function referral_string(session, job) {
    ...
    referral && session.referral != 'Direct' && session.referral != 'http://opower.com'  && referral.push( session.referral);
    var string = referral.join(' ');
    log('APPLICATION', job ? job.title + ' ' + job.location : '', string /*, 'Pages:', session.pageCount || 0 */);
    return string;
}
...
function addHandlers(options) {
    ...
    Server.locals({ sessionHandler: sessionHandler });
