...
function setReferral(req) {
    ...
    if (req.session.referral) {
        req.session.referral.replace(/'"/g, '');
    }
}
...
function sessionHandler(req) {
    ...
    if (req.session) {
        if (!req.session.pageCount) {
            ...
            req.session.insider && gaq_events.push("_gaq.push(['_trackEvent', 'insider', '" + req.session.insider + "', '" + req.session.referral + "']);");
            req.session.jobboard && gaq_events.push("_gaq.push(['_trackEvent', 'jobboard', '" + req.session.jobboard + "', '" + req.session.referral + "']);");
